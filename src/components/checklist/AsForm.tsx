...
import { AS_DOCS, AsState, asItens, emptyAs } from "@/utils/checklistSchema";
...
import { exportAsPDF } from "@/utils/checklistExport";
import { exportAsDOCX } from "@/utils/checklistDocx";
...
export const AsForm = ({ cnpj, operacaoId, razaoSocial }: Props) => {
  const { state, patch, loading, saving } = useChecklist<AsState>({
    cnpj,
    operacaoId,
    checklistType: "as",
    empty: emptyAs,
  ...
  const marcados = useMemo(
    () => Object.values(state.docs ?? {}).filter(Boolean).length,
    [state.docs],
  );
  ...
  const setDoc = (num: string, v: boolean) =>
    patch({ docs: { ...state.docs, [num]: v } });
    ...
    <div className="grid gap-4 lg:grid-cols-2 items-start">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-xs text-muted-foreground">
            Análise de Operação (AS) — FIDC Mais Todos
          </p>
        ...
        <SectionCard numero={1} titulo="Identificação da franquia" complete={idComplete}>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Ex: CLINICA AMOR SAUDE ITABUNA LTDA"
            />
          </div>
        ...
        <SectionCard numero={2} titulo="Itens verificados" complete={itensComplete}>
          <div className="space-y-1.5">
            <Label className="text-xs">Representantes legais</Label>
            <PeopleCards
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Banco 000 - Ag 0000 - CC 00000000-0"
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Fiel depositário + dados bancários</Label>
            <Input
              value={state.fielDepositario}
              disabled={state.fielNA}
              onChange={(e) => patch({ fielDepositario: e.target.value })}
              placeholder="Nome / Banco 000 - Ag 0000 - CC 00000000-0"
            />
            <label className="flex items-center gap-2 text-xs text-muted-foreground pt-1">
              <Checkbox
                checked={state.fielNA}
                onCheckedChange={(v) =>
                  patch({
                    fielNA: !!v,
                    fielDepositario: v ? "Não se aplica" : "",
                  })
                }
              />
              Não se aplica
          ...
          {/* Dados da operação — ordem de Tab = ordem de saída no documento */}
          <div className="pt-2 space-y-2 rounded-md bg-muted/40 p-3">
            ...
            <div className="space-y-1">
              ...
              <PillGroup
                ...
                onChange={(v) =>
                  patch({ opCarenciaTipo: v as AsState["opCarenciaTipo"] })
                }
              />
            </div>
            {state.opCarenciaTipo === "total_e_principal" ? (
            ...
            <div className="rounded-md bg-primary text-primary-foreground text-[11px] font-semibold text-center py-2 tracking-wide">
              CRÉDITO GARANTIA DE RECEBÍVEIS
              <span className="font-normal opacity-70"> — incluído automaticamente</span>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs">Média dos 12 meses (recebíveis crédito)</Label>
            <Input
              value={state.media12}
              onChange={(e) => patch({ media12: mascaraMoeda(e.target.value) })}
              placeholder="R$ 000.000,00/mês"
              inputMode="numeric"
            />
          </div>
        </SectionCard>
        ...
        <SectionCard
          numero={3}
          titulo={`Documentos anexos (${marcados}/${AS_DOCS.length})`}
          complete={docsComplete}
        >
          <p className="text-[11px] text-muted-foreground">
            Itens não marcados sairão como <strong>PENDENTE</strong> no PDF.
          </p>
          <div className="flex gap-2">
            <Button
              ...
              onClick={() =>
                patch({
                  docs: Object.fromEntries(AS_DOCS.map((_, i) => [String(i + 1), true])),
                })
              }
            >
              Marcar todos
          ...
          {AS_DOCS.map((nome, i) => {
            ...
            return (
              ...
              >
                <Checkbox
                  checked={checked}
                  onCheckedChange={(v) => setDoc(num, !!v)}
                  className="mt-0.5"
                />
        ...
        <ExportButtons
          onPdf={() => exportAsPDF(state, razaoSocial)}
          onDocx={() => exportAsDOCX(state, razaoSocial)}
          pdfLabel="Baixar PDF do Checklist AS"
        />
      </div>

      {/* Preview fixo — rolagem independente do formulário */}
      <div className="lg:sticky lg:top-4 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
        <DocumentPreview
          ...
          subtitulo="Análise de Operação (AS)"
          identificacao={[
            ["RAZÃO SOCIAL COMPLETA", state.razaoSocial],
            ["CNPJ", state.cnpj],
          ]}
          itens={asItens(state)}
          docs={AS_DOCS.map((nome, i) => {
            const num = String(i + 1);
            return {
              label: nome.toUpperCase(),
              status: state.docs[num] ? `Documento anexo – ${num}` : "PENDENTE",
            };
          })}
        />
      </div>
